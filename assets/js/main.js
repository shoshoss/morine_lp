(() => {
	...
	function initBookingConfirmation() {
		// 予約前確認モーダルの表示頻度
		// false: 動作確認用。予約CTAクリックごとにモーダルを表示
		// true : 公開用。同一セッションでは確認後1回のみ表示
		const DISCLAIMER_ONCE_PER_SESSION = true;
		...
		function isAccepted() {
			if (!DISCLAIMER_ONCE_PER_SESSION) {
				return false; // 動作確認用: 確認済みデータが残っていても無視して毎回表示
			}
			try {
